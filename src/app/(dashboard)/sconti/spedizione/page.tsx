...
import { ArrowLeft, RefreshCw, Search, Truck, Users, Calendar, Tag, MapPin, Globe } from "lucide-react";
import Link from "next/link";
import { Badge } from "@/components/ui/badge";
import { Checkbox } from "@/components/ui/checkbox";

function SpedizioneGratuitaContent() {
  ...
  const [formData, setFormData] = useState({
    ...
    codice: "",
    titolo: "",
    paesi: "tutti", // tutti, specifici
    paesiSelezionati: [] as string[],
    escludiTariffe: false,
  ...
  const handleSalva = () => {
    console.log("Salva spedizione gratuita:", formData);
    router.push("/sconti");
  };
    ...
    <div className="flex gap-6">
      <div className="flex-1 max-w-3xl">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Spedizione gratuita</h1>
            <p className="text-sm text-gray-500">Offri la spedizione gratuita ai tuoi clienti</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            {formData.metodo === "codice" && (
              <div className="mt-4">
                ...
                <div className="flex gap-2">
                  <input type="text" value={formData.codice} onChange={(e) => setFormData({ ...formData, codice: e.target.value.toUpperCase() })} placeholder="es. FREESHIP" className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg outline-none" />
            ...
            {formData.metodo === "automatico" && (
              <div className="mt-4">
                ...
                <input type="text" value={formData.titolo} onChange={(e) => setFormData({ ...formData, titolo: e.target.value })} placeholder="es. Spedizione gratuita sopra 50 EUR" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg outline-none" />
              </div>
            )}
          </div>
          ...
          {/* Paesi */}
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h2 className="font-semibold text-gray-900 mb-4">Paesi di spedizione</h2>
            <div className="space-y-3">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="paesi" value="tutti" checked={formData.paesi === "tutti"} onChange={(e) => setFormData({ ...formData, paesi: e.target.value })} className="w-4 h-4 text-blue-600" />
                <Globe size={16} className="text-gray-500" />
                <span>Tutti i paesi</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="paesi" value="specifici" checked={formData.paesi === "specifici"} onChange={(e) => setFormData({ ...formData, paesi: e.target.value })} className="w-4 h-4 text-blue-600" />
                <MapPin size={16} className="text-gray-500" />
                <span>Paesi specifici</span>
              </label>
            </div>
            {formData.paesi === "specifici" && (
              <div className="mt-4">
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                  <input type="text" placeholder="Cerca paesi..." className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm" />
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <Badge variant="outline" className="cursor-pointer hover:bg-gray-100">Italia</Badge>
                  <Badge variant="outline" className="cursor-pointer hover:bg-gray-100">Francia</Badge>
                  <Badge variant="outline" className="cursor-pointer hover:bg-gray-100">Germania</Badge>
                  <Badge variant="outline" className="cursor-pointer hover:bg-gray-100">Spagna</Badge>
                </div>
              </div>
            )}
            <div className="mt-4 pt-4 border-t border-gray-100">
              <div className="flex items-center gap-3">
                <Checkbox checked={formData.escludiTariffe} onCheckedChange={(checked) => setFormData({ ...formData, escludiTariffe: checked as boolean })} />
                <span className="text-sm">Escludi tariffe di spedizione superiori a un certo importo</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                <Checkbox checked={formData.combinazioni.altriScontiSpedizione} onCheckedChange={(checked) => setFormData({ ...formData, combinazioni: { ...formData.combinazioni, altriScontiSpedizione: checked as boolean } })} />
                <span className="text-sm">Altri sconti sulla spedizione</span>
              </div>
            </div>
          </div>
      ...
      <div className="w-80 flex-shrink-0">
        <div className="bg-white rounded-xl border border-gray-200 p-6 sticky top-6">
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              <Truck size={16} className="text-teal-500 mt-0.5" />
              <div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs text-gray-500">Paesi</p>
                <p className="text-sm font-medium">{formData.paesi === "tutti" ? "Tutti i paesi" : "Paesi selezionati"}</p>
              </div>
            </div>
            {formData.requisitiMinimi === "importo" && formData.importoMinimo && (
              <div className="flex items-start gap-3">
                <Tag size={16} className="text-gray-400 mt-0.5" />
                <div>
                  <p className="text-xs text-gray-500">Minimo ordine</p>
                  <p className="text-sm font-medium">{formData.importoMinimo} EUR</p>
                </div>
              </div>
            )}
            <div className="flex items-start gap-3">
          ...
          <div className="mt-6 pt-4 border-t border-gray-200">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              {formData.combinazioni.altriScontiSpedizione && <Badge variant="outline" className="text-xs">Spedizione</Badge>}
            </div>
          </div>
          <div className="mt-4 p-3 bg-teal-50 rounded-lg">
            <p className="text-xs text-teal-700">
              <strong>Suggerimento:</strong> La spedizione gratuita e uno dei migliori incentivi per aumentare le conversioni!
            </p>
          </div>
        </div>
      </div>
...
export default function SpedizioneGratuitaPage() {
  return (
